...
class Solution {
    public DLLNode sortAKSortedDLL(DLLNode head, int k) {
        ...
        // Create a min heap (priority queue) to store DLL nodes
        ...
        // Now process the heap and keep placing nodes in their correct positions
        while (!minHeap.isEmpty()) {
            ...
            if (newHead == null) {
                ...
                // Attach to the sorted part
                last.next = minNode;
                minNode.prev = last;
                last = minNode;
            }

            // Add the next node from the unsorted list to the heap (if any)
            if (head != null) {
                minHeap.add(head);
                head = head.next;
            }
