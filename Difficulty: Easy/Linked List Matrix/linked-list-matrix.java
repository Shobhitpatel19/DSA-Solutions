...
class Solution {
    static Node construct(int mat[][]) {
        // Add your code here.
        int n = mat.length;
        if (n == 0) return null; // Handle edge case if matrix is empty
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (j + 1 < n) {
                    nodes[i][j].right = nodes[i][j + 1]; // Link to right
                }
                if (i + 1 < n) {
                    nodes[i][j].down = nodes[i + 1][j];  // Link to down
                }
            }
        }

        // The head of the 2D linked list is the first node in the matrix
        return nodes[0][0];
    }
}